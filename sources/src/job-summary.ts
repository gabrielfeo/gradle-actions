...
function renderBuildScan(result: BuildResult): string {
    ...
    if (result.buildScanUri) {
        return renderBuildScanBadge('Build Scan®', 'Build Scan published', '06A0CE', result.buildScanUri)
    }
    return renderBuildScanBadge('Not published', 'Build Scan not published', 'lightgrey', 'https://scans.gradle.com')
}

function renderBuildScanBadge(text: string, altText: string, outcomeColor: string, targetUrl: string): string {
    const encodedText = encodeURIComponent(text)
    const badgeUrl = `https://img.shields.io/badge/${encodedText}-${outcomeColor}?logo=Gradle`
